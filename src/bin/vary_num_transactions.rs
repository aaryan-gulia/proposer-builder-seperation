...
fn main() {
    vary_num_transactions();
}

fn vary_num_transactions() {
    ...
    let pb = ProgressBar::new((NUM_TRANSACTIONS) as u64);
    let num_transactions_vec: Vec<u32> = (80..=NUM_TRANSACTIONS).collect(); // Parallelize the outer loop iterating over num_builders
    num_transactions_vec
        .into_par_iter()
        .for_each(|num_transactions| {
            for characteristic in 5..=5 {
                let characteristic: f64 = characteristic as f64 / 5.0;
                let mut builder_vec =
                    initiate_builders(NUM_BUILDERS - MEV_BUILDERS, characteristic);
                ...
                let file_name = format!(
                    "data/pbs_vary_num_transactions/num_transactions={}characteristic={}.csv",
                    num_transactions, characteristic
                );
